...
function RegisterForCourse() {
    ...
    const [selectedDetailSectionId, setSelectedDetailSectionId] =
        useState(null);
    ...
    useEffect(() => {
        let getListCourseById = async () => {
            let datas = await axios.get(
                `http://localhost:8080/courses/${location.state.studentId}/${location.state.majorId}`,
                {
    ...
    useEffect(() => {
        let getCourseSectionRegistered = async () => {
            let datas = await axios.get(
                `http://localhost:8080/course-sections/registered-course-section/${location.state.studentId}/${selectedOption}`
            );
    ...
    useEffect(() => {
        ...
        const theoryLectureId = courseSectionDetail.find(
            (item) => item.type === "LT"
        )?.lectureId;
    ...
    let handleClickCourse = async (majorId) => {
        let datas = await axios.get(
            `http://localhost:8080/course-sections/${majorId}`
        );
    ...
    let handleClickCourseSection = async (sectionId) => {
        let datas = await axios.get(
            `http://localhost:8080/course-sections/detail-course-section/${sectionId}`
        );
    ...
    let handleRegisteredQuantity = async (sectionId) => {
        let datas = await axios.get(
            `http://localhost:8080/course-sections/student-enrollment-numbers/${sectionId}`
        );
    ...
    const generateOptions = (startYear, numYears) => {
        ...
        for (let year = startYear; year <= endYear; year++) {
            ...
            options.push(
                <option
                    key={`HK1(${yearString})`}
                    value={`HK1 (${yearString})`}
                >
                    HK1 ({yearString})
                </option>,
                <option
                    key={`HK2(${yearString})`}
                    value={`HK2 (${yearString})`}
                >
                    HK2 ({yearString})
                </option>,
                <option
                    key={`HK3(${yearString})`}
                    value={`HK3 (${yearString})`}
                >
                    HK3 ({yearString})
                </option>
            );
    ...
    const handleRegister = async () => {
        if (
            ...
            !selectedSectionId ||
            selectedOption == "" ||
            !selectedDetailSectionId
        ) {
            toast.error(
                "Vui lòng chọn học kì, môn học, lớp học phần và nhóm thực hành."
            );
            return;
        }
        try {
            const response = await axios.post(
                `http://localhost:8080/grades?studentId=${location.state.studentId}&courseSectionId=${selectedSectionId}&courseId=${selectedCourseId}&lectureTheoryId=${lectureTheoryId}&lecturePracticeId=${selectedDetailSectionId}&semester=${selectedOption}`
            );
            ...
            if (response.data === "Success") {
                ...
                response.data ===
                "You have not completed the prerequisite course"
            ) {
                toast.error(
                    "Bạn chưa vượt qua môn học tiên quyết của môn học này!"
                );
            } else {
            ...
            <div className="choose-register">
                <div className="register-period">
                    ...
                    >
                        ...
                        {generateOptions(
                            parseInt(courseYear.split("-")[0], 10),
                            5
                        )}
                    </select>
                </div>
                            ...
                            {listCourseSection.map((item, index) => {
                                ...
                                return (
                                    <tr
                                        key={index}
                                        onClick={() =>
                                            handleClickCourseSection(item.id)
                                        }
                                        style={{
                                            backgroundColor:
                            ...
                            {courseSectionDetail.map((item, index) => {
                                return (
                                    <tr
                                        ...
                                        onClick={() =>
                                            handleClickDetailSection(
                                                item.lectureId
                                            )
                                        }
                                    >
                                        <td>{index + 1}</td>
                                        <td>{`${getDay(item.dayOfWeek)} (${
                                            item.shift
                                        })`}</td>
                                        ...
                                        <td>
                                            {item.type === "TH"
                                                ? `${item.studentEnrollmentNumbers}/30`
                                                : `${item.studentEnrollmentNumbers}`}
                                        </td>
                                    </tr>
                                );
                            })}
